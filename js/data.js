...
export async function fetchArticles({ category = '', search = '', sort = 'newest', max = 24 } = {}) {
  let q = query(collection(db, 'articles'), where('status', '==', 'published'), orderBy('publishedAt', 'desc'), limit(max));
  const snap = await getDocs(q);
  let rows = snap.docs.map(mapArticle);
  if (!rows.length) {
    rows = SEED_ARTICLES.map((a, i) => ({ ...a, id: `seed-${i}`, publishedAtDate: new Date(a.publishedAt), html: markdownToHtml(a.bodyMarkdown) }));
  }
...
export async function fetchArticleBySlug(slug) {
  ...
  if (seed) return { ...seed, id: `seed-${slug}`, publishedAtDate: new Date(seed.publishedAt), html: markdownToHtml(seed.bodyMarkdown) };
  return null;
}
...
export async function seedInitialDataIfEmpty() {
  ...
  await Promise.all(SEED_ARTICLES.map((a) => setDoc(doc(collection(db, 'articles')), {
    ...a,
    bodyHtml: markdownToHtml(a.bodyMarkdown),
    createdAt: serverTimestamp(),
    updatedAt: serverTimestamp(),
